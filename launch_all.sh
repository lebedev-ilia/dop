...
if ! command -v cloudflared &> /dev/null; then
    ...
    echo "Запустите: ./install_cloudflared.sh"
    echo "Или установите вручную. Продолжить без cloudflared? (y/n): "
    read -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        exit 1
...
if [ "$SKIP_CLOUDFLARED" = "false" ]; then
    ...
    echo "⚠ Пропущен запуск cloudflared (не установлен)"
fi
...
echo ""
echo "Как выйти из tmux сессии (вернуться в обычный терминал):"
echo ""
echo "  Способ 1 (рекомендуется для Mac/SSH):"
echo "    Внутри tmux нажмите: Ctrl+B, затем введите: :detach"
echo "    (или просто: :detach и Enter)"
echo ""
echo "  Способ 2 (если Ctrl+B работает):"
echo "    Ctrl+B, затем D                 - отключиться (detach)"
echo ""
echo "  Способ 3 (из другого терминала):"
echo "    tmux detach -s <session_name>   - отключиться от конкретной сессии"
echo ""
echo "  ВАЖНО: НЕ используйте Ctrl+C или exit - это остановит процесс!"
echo ""
echo "  Если Ctrl+B не работает, используйте Способ 1 или 3"
echo ""
...
echo ""
echo "Утилиты:"
echo "  ./kill_port.sh <port>           - остановить процесс на порту"
echo "  ./kill_port.sh 9090              - остановить Prometheus (если порт занят)"
echo "  ./install_cloudflared.sh         - установить cloudflared (если не установлен)"
