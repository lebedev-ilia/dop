...
def save_progress_upload(success_batchs):
    progress_path = os.path.join(project_root, "_yt_dlp", ".cache", "progress_upload.json")
...
def main():
    uploader = HuggingFaceUploader(
        ...
        token="",
        cache_dir=os.path.join(project_root, "_yt_dlp", ".cache")
    )
    
    YT_DLP_RESULTS_DIR = os.path.join(project_root, "_yt_dlp", ".results")
    ...
    meta_folder_name = f"meta_{date}_yt_dlp"
    ...
    while True:
        ...
        new_batchs = uploader.check_for_new_batches(success_batchs + pending_batches, YT_DLP_RESULTS_DIR)
        ...
        if should_commit:
            ...
            while batches_to_process and commits_left > 0:
                ...
                for batch_name in current_batch_chunk:
                    try:
                        with open(os.path.join(YT_DLP_RESULTS_DIR, batch_name), "r") as f:
                            ...
                            for cat, ids in cat2ids.items():
                                if video_id in ids:
                                    ...
                                    if hf_path in uploader.current_repo_files:
                                        print(f"Metadata for video {video_id} in category {cat} already exists")
                                        continue
                ...
                if files_to_upload:
                    try:
                        ...
                        print(f"Current repo files: {len(uploader.current_repo_files)}")
